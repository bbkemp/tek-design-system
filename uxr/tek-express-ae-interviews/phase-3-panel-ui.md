...
- **Group tests by pattern source**; tooltips showing required pattern per test. **Dallas/Jesse** → [ae-dallas-jesse-setup-flow-friction](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-setup-flow-friction.md)
- **Test names match spec test IDs.** **Dallas/Jesse** → same chunk.
...
- **Smart mutually-exclusive selection** — kill the "check one before you uncheck the other" dance; non-exclusive toggles that can't both be off. **Dallas/Jesse** → [ae-dallas-jesse-setup-flow-friction](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-setup-flow-friction.md)
...
- **Fix the pre-recorded waveform workflow** (three-way corroborated): filename convention carries metadata customers never follow — "If I have to remember what that name's supposed to be… good luck" (**Chris B**); AEs connect an AWG just to harvest correct names (**Dallas/Jesse**: "we've made it so hard… the messages were terrible"); DDR failures round-trip through live mode (**Sam**). Fix shape: point-and-rename dialog mapping files → signals ("I can tell it this is my PRBS 0 waveform on channel one… Just make it rename it for me"). → [ae-chrisb-setup-waveforms-and-run-modes](../../corpus/sources/tek-express/walkthroughs/ae-chrisb-setup-waveforms-and-run-modes.md), [ae-dallas-jesse-setup-flow-friction](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-setup-flow-friction.md), [ae-sam-ddr-memory-feedback](../../corpus/sources/tek-express/walkthroughs/ae-sam-ddr-memory-feedback.md)
- **No live↔pre-recorded settings trap** — "why can't I control this from pre-recorded mode in the first place?" (**Sam**); settings locked in pre-recorded mode force switching back and forth (**Chris B**). → chunks above
- **De-embed/filter assignment must be scannable** — "you kind of get this just big giant list of like file pathways"; group filters by measurement, picker-style. **Sam** → [ae-sam-ddr-memory-feedback](../../corpus/sources/tek-express/walkthroughs/ae-sam-ddr-memory-feedback.md)
...
- **Math waveforms as sources.** **Dallas/Jesse** → [ae-dallas-jesse-instruments-sessions-results-debug](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-instruments-sessions-results-debug.md)
- **Keep the derived per-row context** ("Lane Zero, PRTS 7, P0 — way better than it used to be"). **Dallas/Jesse** → same chunk.
- **Acquisition-speed opportunities (backend, UX-visible):** don't reset instruments between acquisitions ("We would speed ourselves up 30 and 40%"); parallel lane acquisition ("4 minutes instead of 30"). **Dallas/Jesse** → [ae-dallas-jesse-compliance-mode-and-test-selection](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-compliance-mode-and-test-selection.md)
...
- **"This will change A, B, C — continue/cancel" dialogs** for silent cascading setting changes ("You thought you were changing one field before fields change"). **Dallas/Jesse** → [ae-dallas-jesse-setup-flow-friction](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-setup-flow-friction.md)
- **Global/local paradigm for signal validation** (modeled live on the scope's measurement settings) with "run anyway but flag validation failure" replacing the confusing third choice. **Dallas/Jesse** → [ae-dallas-jesse-compliance-mode-and-test-selection](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-compliance-mode-and-test-selection.md)
...
- **"Rethink the tabs — were they necessary, or were they just there?"**; "this menu feeds the next menu" — key info in both selection and detail. **Dallas/Jesse** → [ae-dallas-jesse-setup-flow-friction](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-setup-flow-friction.md)
...
- **Connection/channel column in Status.** **Dallas/Jesse** → [ae-dallas-jesse-instruments-sessions-results-debug](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-instruments-sessions-results-debug.md)
...
- **Selective rerun with result replacement** — "If the test took eight hours to run… let me run this one test. Give me the choice to replace the results" (**Dallas/Jesse**); rerun-failed-only with append/replace choice; right-click debug menu on a failed result (rerun compliance / rerun user-defined / export scope session). → [ae-dallas-jesse-instruments-sessions-results-debug](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-instruments-sessions-results-debug.md)
- **Column add/remove + TekScope results-table configure parity** (team-endorsed); spec-version column. **Dallas/Jesse** → same chunk.
- **Plots from analysis tools surfaced in results** (and reports). **Dallas/Jesse** → same chunk. (Depends on the chart-library decision — Open decision 5.)
...
"The super set of stupid" — design for PCIe and everything else follows (**Dallas/Jesse**); ~50% of the business (**product leader**); "inarguably the one to focus on first" (**Will**). → [ae-dallas-jesse-setup-flow-friction](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-setup-flow-friction.md), [ae-will-teverovsky-session-files-options-and-program-context](../../corpus/sources/tek-express/walkthroughs/ae-will-teverovsky-session-files-options-and-program-context.md)
...
- **Presets as list rows, not tabs.** **Dallas/Jesse** → [ae-dallas-jesse-compliance-mode-and-test-selection](../../corpus/sources/tek-express/walkthroughs/ae-dallas-jesse-compliance-mode-and-test-selection.md)
...
- **Validation plan required** — Sam's session was 9:35 truncated (bench lacked the plugin; TekExpress hung at launch); a follow-up was agreed in-session. Open decision 9. All → same chunk.
