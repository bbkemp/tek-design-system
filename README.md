...
**Corpus vs. RAG.** The corpus is the *asset* — the durable markdown. RAG is one *technique* for consuming it (others: Claude Code reading files directly, grep, a future retrieval layer). Authoring is shipped; retrieval is Phase 2.

Authoring is shipped: drop a pile of mixed files into [`corpus/_inbox/`](./corpus/_inbox/), run `/corpus-intake`, and a routing plan sorts them into the right per-product `uploads/<class>/` folder; per-class processing skills (`/document-screens`, `/document-pdf`, `/document-walkthrough`, etc.) then turn them into the corpus markdown. Retrieval (vector DB, embeddings, MCP) is Phase 2.
