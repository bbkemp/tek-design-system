...
├── rag/                      Knowledge corpus — structured markdown about Tek products
│   ├── sources/              per-product corpora (screens, walkthroughs, docs, hardware, code, api)
...
├── .claude/skills/           Repo-scoped Claude Code skills (rag-intake, document-*, prototype-qa, etc.)
...
Alongside the design system, this repo holds a structured-markdown corpus of the legacy Tek products being modernized — one folder per product under [`rag/sources/`](./rag/sources/), with screens, walkthroughs, hardware views, manuals, APIs, and code snapshots all in a locked markdown format. Interpretation reports (DS gap analyses, redesign briefs, competitive comparisons) live separately under [`audits/`](./audits/) and are dated, disposable.

Authoring is shipped: drop a pile of mixed files into [`rag/_inbox/`](./rag/_inbox/), run `/rag-intake`, and a routing plan sorts them into the right per-product `uploads/<class>/` folder; per-class processing skills (`/document-screens`, `/document-pdf`, `/document-walkthrough`, etc.) then turn them into the corpus markdown. Retrieval (vector DB, embeddings, MCP) is Phase 2.

Read [`rag/README.md`](./rag/README.md) for the layout, workflow, and corpus-vs-audit boundary; [`audits/README.md`](./audits/README.md) for the audit types.
