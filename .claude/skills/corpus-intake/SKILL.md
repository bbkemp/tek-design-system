...
6. **Quick content scan, not full processing.** Always peek at content to verify what the file *is* before naming the destination — read the first 1-3 pages of every PDF, sample 2-4 photos from any folder of generic-named images, read the first page of every .docx. The peek informs the routing plan (better subject names, accurate audit slugs); it does not replace the downstream processing skills (no OCR, no transcription, no full text extraction). Keep the peek bounded — page count not chapter count, sample not exhaustive, header not body.
...
### 1.5. Content scan — peek before classifying

For every file (and every distinct folder of similar files) in the inbox, do a bounded content scan before building the classification. The scan tells you what the file actually *is* — not what the filename suggests — so subject names and audit slugs reflect content, not guessing.

| File type | Scan |
|---|---|
| `.pdf` | Read first 1-3 pages via the Read tool's `pages` parameter. Capture: title, cover/TOC headings, comparison subjects (for audits), product name + version. |
| `.docx`, `.doc` | Read the first page or first 2-3 paragraphs. Capture: title, meeting/document type, participants if it's a transcript. |
| `.pptx`, `.key` | Read the title slide if accessible. Capture: deck title, comparison subjects, meeting purpose. |
| `.png`/`.jpg`/`.jpeg`/`.heic` in a folder of generic-named photos (IMG_NNNN, Screenshot YYYY-MM-DD, etc.) | Sample 2-4 photos via the Read tool. Capture: visible vendor logos, software titles, SKUs / product names on-screen, scope brand. Pick samples spread across the filename range, not just the first few. |
| `.xlsx`, `.csv` | Read the header row + first 5 rows. Capture: column names, what's being compared. |
| `.md` | Read first 30 lines. Capture: title, intent, whether it's an audit-draft. |
| `.yaml`, `.yml`, `.json` (API spec candidates) | Read first 50 lines. Capture: `info.title`, `info.version`, `swagger`/`openapi` version. |
| `.chm` | Skip the scan — binary format, treat as a manual based on filename. |
| Audio / video | No scan (refused at intake anyway). |

Use the scan output to:
- **Refine the audit slug.** A `CompetitiveAnalysis.pdf` whose cover says "TekExpress DisplayPortTx 2.1 vs Keysight D9040DPPC" gets `audits/competitive/2026-06-25-tek-express-vs-keysight-d9040dppc/`, not a generic `competitive-analysis` slug.
- **Pick the right subject SKU.** A folder of `IMG_NNNN.jpg` files whose samples show a Keysight Infiniium running "D9040USBC USB4 Test Application" routes to `keysight-d9040`, not "Ask the user."
- **Confirm the class.** A `.docx` named `Meeting Notes 2026-06-23.docx` whose first paragraph reads like a meeting transcript routes to `uploads/transcripts/`; one whose first page reads like a feature spec might warrant a `uploads/artifacts/` override and a flag in the plan.
- **Catch misroutes early.** A file named `manual.pdf` whose first page is actually a comparison matrix gets routed as an audit, not corpus.

The peek is bounded: page count not chapter count, sample not exhaustive, header not body. If a scan would take more than ~30 seconds (e.g. reading 200 pages of a manual), stop after the cover and TOC.

Include scan findings in the routing plan as a one-line "Why:" column so the user can verify your inference before saying `go`.
...
A Markdown table with columns: `Original path | Detected product | Class | Destination | Why | Action`.

The `Why` column is the one-line scan finding from Section 1.5 that justifies the routing (e.g. "Cover page: TekExpress vs Keysight D9040DPPC", "Sampled IMG_7010 shows Keysight Infiniium D9040 family", "First paragraph reads as meeting transcript"). Lets the user verify the inference before saying `go`.
...
| Original path | Product | Class | Destination | Why | Action |
|---|---|---|---|---|---|
| `Tek Express Screens/0. Options.png` | tek-express | photos | `corpus/sources/tek-express/uploads/photos/0._Options.png` | Sampled image shows TekExpress Options ▼ menu | move |
| ... | ... | ... | ... | ... | ... |
