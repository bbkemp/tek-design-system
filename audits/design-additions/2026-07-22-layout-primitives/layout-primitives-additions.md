...
|---|---|---|---|
| Stack | `8376:373` | 7 | Gap = s02, s04, s05, s06, s08, s09, s11 |
| Row | `8377:373` | 7 | Gap = s02, s04, s05, s06, s08, s09, s11 |
...
1. **`s04` added to the Stack/Row gap axis.** The draft listed six steps (s02, s05, s06, s08, s09, s11). The sign-in reference pages — the first refactor target — use `--tek-spacing-s04` (6px) for the label→input field gap ([page-layout.css:77](../../../prototypes/sign-in/page-layout.css)). A gap axis that can't express the reference pages fails its own "capped to real-world steps" test. s04 is a real-world step; added.
...
- **Slots cannot be created via the plugin API** (`createSlot` / `convertToSlot` absent). Each variant contains a `Content` frame carrying the bound gap — these need **manual conversion to slots** in the Figma UI (right-click → convert to slot) before the sets are published. 26 frames total, one per variant.
...
- [ ] Convert the 26 `Content` frames to slots (Stack 7, Row 7, Grid 9, Page 3)
...
- [ ] Publish the library update when happy
