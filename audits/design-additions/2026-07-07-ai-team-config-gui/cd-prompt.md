...
>    - `tek-express.html`, `bench.html`, `my-tek.html` — the three reference prototypes as self-contained files. These are the visual ground truth: Claude Design reviews them itself (it's required to read and report on them before designing — §3/§4).
>    - Optional: a screenshot of each prototype, as an extra visual anchor. Nice to have, not required — the HTML files are the authoritative reference.
>
