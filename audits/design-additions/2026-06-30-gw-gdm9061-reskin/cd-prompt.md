...
4. **The processed manual — `docs/user-manual/`.** The GW Instek GDM-9060/9061 User Manual (Rev. G) is now in the corpus as 16 locked-format chunks, and **every screen's `## Manual references` section cites the chunks that back it** (no placeholders remain). Read [`docs/user-manual/_index.md`](https://github.com/bbkemp/tek-design-system/blob/main/corpus/sources/gw-gdm9061/docs/user-manual/_index.md) for the map, then follow each screen's Manual references into the relevant chunk when you need the authoritative spelling of a label, the exact softkey set for a function, or the behavior behind a control. The manual is the deepest source of truth for verbatim text — but it does **not** change the reskin scope: you are still restyling paint only (§1), not re-deriving the UI from the manual. Use it to get labels/values exactly right, not to add or relayout anything.
   - The manual **resolves several of the screens' original confidence-note uncertainties** — e.g. `status-bar.md` (pp. 21-23) gives the full top-bar icon legend (LOC/Remote, the RS-232/USB-CDC/USB-TMC/LAN/GPIB interface icon, error, rear-panel switch, Shift, 1st/2nd function-menu, Digital I/O User/4094, flash-drive, sound, internet, clock). Prefer the manual's wording where a screen note flagged a value as uncertain.
   - Note the manual's *own* pending tail (full SCPI command set, specifications, etc. — listed in the doc `_index.md`): it has no bearing on this reskin.

5. **Honor the `## Confidence notes`.** Where the corpus still flags a value as uncertain (e.g. the exact MAC address, stats-box exponents, the `OverLoad` behavior that the per-function manual sections don't cover), reproduce what's written — don't "fix" it. If a screen note has since been resolved by a manual chunk (see #4), use the manual's value.

If you can't cleanly retrieve a screen's verbatim text, its image, or a cited manual chunk, stop and re-read `index.md` before continuing.
