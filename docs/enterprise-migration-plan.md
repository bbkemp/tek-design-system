...
- **2026-07-30 (partial cutover — evening)** — Bryan had the org-admin conversation (team + Claude App in motion). CODEOWNERS merged (org PR #1). **Real issue migration DONE**: all 98 issues live on the org repo (35 open / 63 closed, verified; org numbering interleaves PRs, mapping at [`docs/enterprise-issue-mapping.json`](./enterprise-issue-mapping.json)). `main` merge-synced to org (merge commit, SHAs preserved — org PR #3). **Interim rule until final cutover: new/updated ISSUES happen on the org repo; code/PRs stay on bbkemp.** Remaining for cutover day: Vercel + MCP rewiring, `DATABASE_URL`/`VOYAGE_API_KEY` secrets, workflow re-enable, package scope rename, Figma plugin retarget, final main sync, archive bbkemp with pointer.
